...
let title = "Create Group Profile";
let role = null;
...
async function onSubmit(values) {
  try {
    ...
    let outputObj = {
      menuAuthorlist: inputObj.map(menuId => ({
        menuId: menuId.toString(),
        menuStatus: "A"
      }))
    };

    values.groupAuthor = outputObj;

    await rolesStore.create(JSON.stringify(values));

    if(rolesStore.get_res_code === 'WA00000'){
      message = "สร้างบทบาทผู้ใช้เสร็จสิ้น";
      await router.push("/role-management");
      alertStore.success(message);
    } else {
      message = '"' + rolesStore.get_res_desc + '"' + " การสร้างบทบาทผู้ใช้เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง";
      await router.push("/role-management");
...
<template>
    ...
    <Breadcrumb link_menu="/role-management" name_menu="จัดการสิทธิ์ผู้ดูแลระบบ" title="สร้างสิทธิ์ผู้ดูแลระบบ" />
                ...
                <tbody class="text-center">
                    <template v-if="row_tables">
                        <tr v-for="row in row_tables.menus" :key="row.id">
                            ...
                            <td class="whitespace-normal p-3"><Field class="checkbox checkbox-sm checkbox-primary" as="input" name="menuId" type="checkbox" :value="row.menuId" /></td>
                        </tr>
