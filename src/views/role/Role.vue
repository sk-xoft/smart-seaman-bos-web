...
import { useAuthStore, useRolesStore } from '@/stores';
...
const { roles, perPage } = storeToRefs(rolesStore);

rolesStore.refreshData();
...
async function onSubmit(values) {
  try {
    ...
    if( !values.groupStatus ){
        values.groupStatus = null;
    }
    
    rolesStore.getSearchRoles(values);
...
<template>
    ...
    <Breadcrumb link_menu="/admin-management" name_menu="จัดการสิทธิ์ผู้ดูแลระบบ" title="ค้นหา" />
        ...
        <div class="card-body p-5 pb-9">
            ...
            >
                ...
                <div class="grid grid-cols-2 items-center gap-7 w-full">
                    <label for="groupName" class="flex flex-col w-full">
                        <span class="mb-2">ชื่อบทบาท</span>
                        <Field
                            id="groupName"
                            name="groupName"
                            type="text"
                    ...
                    <label for="search" class="flex flex-col w-full">
                        ...
                        <Field as="select" v-model="groupStatus" id="groupStatus" name="groupStatus" class="select select-sm input-ghost input-bordered w-full">
    ...
    <div class="flex justify-end mb-3">
        <router-link class="btn btn-sm px-5 btn-primary mb-2" :to="`/role-management/add`">
    ...
    <div class="card overflow-x-auto bg-base-200">
        <div class="card-body pr-7">
            ...
            <thead class="text-center">
                <tr>
                    ...
                    <th class="bg-base-300">ชื่อบทบาท</th>
            ...
            <tbody class="text-center">
                <template v-if="roles">
                    <tr v-for="role in roles.groupList" :key="role.id">
                        <td class="whitespace-normal w-14 p-3">{{ role.groupNum }}</td>
                        <td class="whitespace-normal p-3">{{ role.groupName }}</td>
                        <td class="whitespace-normal p-3">
                            <template v-if="role.groupStatus == 'A'">
                                <span>Active</span>
                            </template>
                            <template v-else>
                                <span>Inactive</span>
                        ...
                        <td class="whitespace-normal p-3">{{ role.createDate ? convertDate(role.createDate) : '-' }}</td>
                        <td class="whitespace-normal p-3">{{ role.updateDate ? convertDate(role.updateDate) : '-' }}</td>
                        <td class="w-28 p-3" style="white-space: nowrap">
                            <router-link :to="`/role-management/edit/${role.groupId}`" class="btn btn-sm btn-primary mr-1"><i class="fa-solid fa-pen-to-square"></i></router-link>
                            <label for="onDelete" @click="passId(role.groupId)" class="btn btn-sm btn-danger btn-delete-user" :disabled="user.isDeleting">
                                <span><i class="fa-solid fa-trash"></i></span>
                            </label>
                        </td>
                    </tr>
                    <tr v-if="roles.loading">
                    ...
                    <tr v-if="Object.keys(roles).length === 0">
                        <td colspan="6" class="text-center">
                            ไม่มีข้อมูล
                        </td>
                    </tr>
        ...
        <template v-if="roles">
            <template v-if="roles.countList > 20">
                <VueTailwindPagination class="pagination"
                :current="currentPage"
                :total="roles.countList"
                :per-page="perPage"
