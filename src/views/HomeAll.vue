...
title = `คนประจำเรือทั้งหมด`;
const { all_detail_user, currentPage, size, lastNum } = storeToRefs(dashboardsStore);

dashboardsStore.refreshAllDataTable();
dashboardsStore.refreshData();
dashboardsStore.getAllDetailUser();

function handlePageChanged(page) {
    dashboardsStore.currentPage = page;
    dashboardsStore.lastNum = dashboardsStore.size * page;
      ...
      <div class="card-body">
          ...
          <table class="table table-striped lg:table-fixed w-full rounded-none text-center">
            ...
            <tbody class="text-center">
                  <template v-if="all_detail_user">
                    <template v-if="!all_detail_user.loading">
                      <template v-for="detail in all_detail_user.items" :key="detail.id">
                    ...
                    <tr v-if="all_detail_user.loading">
                    ...
                    <tr v-if="all_detail_user.error">
                        <td colspan="8" class="text-center">
                            <div class="text-danger">เกิดข้อผิดพลาดไม่สามารถโหลดข้อมูลได้ : {{dashboard_details.error}}</div>
                        </td>
                    </tr>
                    <tr v-if="Object.keys(all_detail_user).length === 0">
                        <td colspan="8" class="text-center">
                            ไม่มีข้อมูล
                        </td>
                    </tr>
        ...
        <template v-if="all_detail_user">
            <template v-if="all_detail_user.totalData > 20">
                <VueTailwindPagination class="pagination"
                    :current="currentPage"
                    :total="all_detail_user.totalData"
                    :per-page="size"
