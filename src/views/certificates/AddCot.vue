...
let title = "เพิ่มประกาศนียบัตร";
let user = null;
let certStartDate = ref('');
let certEndDate = ref('');

const schema = Yup.object().shape({
  documentCode: Yup.string().required("กรุณาเลือกประกาศนียบัตร"),
  certStartDate: Yup.string().required("กรุณาเลือกวันที่ออกประกาศนียบัตร").nullable(),
...
async function onSubmit(values) {
  try {
    ...
    await certificatesStore.create(JSON.stringify(values));

    if(certificatesStore.get_res_code === 'WA00000'){
      message = "เพิ่มประกาศนียบัตรเสร็จสิ้น";
      ...
      console.log(certificatesStore.get_res_code );
      message = '"' + certificatesStore.get_res_desc + '"' + " การเพิ่มประกาศนียบัตรเกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง";
      await router.push(`/certificates/view/${id}`);
      alertStore.error(message);
    }
...
<template>
  ...
  <BreadcrumbView link_menu="/certificates" name_menu="จัดการข้อมูลประกาศนียบัตร" :view_menu="`/certificates/view/${id}`" name_view_menu="ข้อมูลผู้ใช้" title="เพิ่มประกาศนียบัตร" />
  ...
  <template v-if="!(user?.loading || user?.error)">
    <div class="card bg-base-200 mt-5">
      <div class="card-body">
        ...
        <Field type="hidden" name="userMobileUuid" v-model="id" />

        <div class="form-row grid grid-cols-1 gap-5">
            <div class="form-group flex flex-col">
              <label class="mb-2" :class="{ 'is-invalid': errors.documentCode }">ชื่อประกาศนียบัตร <span class="required">*</span></label>
              <Field as="select" v-model="documentCode" id="documentCode" name="documentCode" class="select select-sm input-ghost input-bordered w-full">
                  <option value="" selected>เลือกประกาศนียบัตร</option>
                  <template v-for="option_doc in get_doc_options.cot" :key="option_doc.id">
            ...
            <div class="form-group flex flex-col">
              <label class="mb-2" :class="{ 'is-invalid': errors.certStartDate }">วันที่ออกประกาศนียบัตร <span class="required">*</span></label>
              <Field v-model="certStartDate" type="text" name="certStartDate" v-slot="{ fieldCertStartDate }">
                <VueDatePicker
                  id="certStartDate"
                  name="certStartDate"
                  v-model="certStartDate" 
                  :format="certStartDateFormat" 
                  v-bind="fieldCertStartDate"
                  dark="true"
                  placeholder="เลือกวันที่ออกประกาศนียบัตร" 
                  text-input
                  auto-apply
                ...
                <div class="flex flex-col lg:flex-row gap-7">
                    <Field v-slot="{ field }" v-model="noExpried" name="certEndDateType" type="radio" :value="'N'">
                      <label for="noExpried" class="cursor-pointer flex lg:justify-center items-center gap-3">
                        <input type="radio" id="noExpried" name="certEndDateType" v-bind="field" v-model="certEndDateType" :value="'N'" class="radio radio-primary"/>
                    ...
                    <Field v-slot="{ field }" v-model="expried" name="certEndDateType" type="radio" :value="'A'">
                      <label for="expried" class="cursor-pointer flex lg:justify-center items-center gap-3">
                        <input type="radio" id="expried" name="certEndDateType" v-bind="field" v-model="certEndDateType" :value="'A'" class="radio radio-primary" />
            ...
            <!-- <template v-if="noExpried === 'N' && expried != 'A'">
            ...
            <template v-if="expried === 'A'">
              <div class="form-group flex flex-col" >
                <label class="mb-3" :class="{ 'is-invalid': errors.certEndDate }">วันที่หมดอายุ <span class="required">*</span></label>
                  <Field v-model="certEndDate" type="text" name="certEndDate" v-slot="{ fieldCertEndDate }">
                    <VueDatePicker
                      id="certEndDate"
                      name="certEndDate"
                      v-model="certEndDate" 
            ...
            <div class="form-group flex flex-col">
                <label class="mb-3">อัพโหลดไฟล์ประกาศนียบัตร</label>
                ...
                <div class="input-file-wrapper relative">
                  ...
                  <template v-if="fileCertName">
                    <span class="text-white cursor-pointer absolute top-1 right-3 z-10" @click="resetInput">
                ...
                <Field name="fileCertName" v-model="fileCertName" type="hidden" />
            </div>
          </div>
         <div class="form-group flex mt-10 justify-center items-center gap-5">
            ...
            <router-link :to="`/certificates/view/${id}`" class="btn btn-sm px-5 btn-light text-white px-7">
