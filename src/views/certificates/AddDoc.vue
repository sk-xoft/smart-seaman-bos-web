...
async function onSubmit(values) {
  try {
    ...
    if(certificatesStore.get_res_code === 'WA00000'){
      ...
      message = '"' + certificatesStore.get_res_desc + '"' + "การเพิ่มเอกสารเกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง";
      await router.push(`/certificates/view/${id}`);
      alertStore.error(message);

      console.log(certificatesStore.get_res_code + '__' + certificatesStore.get_res_desc);
    }
...
<template>
  ...
  <BreadcrumbView link_menu="/certificates" name_menu="จัดการข้อมูลประกาศนียบัตร" :view_menu="`/certificates/view/${id}`" name_view_menu="ข้อมูลผู้ใช้" title="เพิ่มเอกสาร" />
    ...
    <div class="card bg-base-200 mt-5">
      <div class="card-body">
        ...
        <div class="form-row grid grid-cols-1 gap-5">
            ...
            <div class="form-group flex flex-col">
                <label class="mb-3">อัพโหลดไฟล์เอกสาร</label>
