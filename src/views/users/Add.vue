...
const schema = Yup.object().shape({
  username: Yup.string().required("กรุณากรอกชื่อผู้ใช้ (ภาษาอังกฤษ)"),
  ...
  password: Yup.string()
    ...
    .concat(user ? null : Yup.string().required("กรุณากรอกรหัสผ่าน"))
    .min(8, "รหัสผ่านจะต้องมีอย่างน้อย 8 ตัวอักษร, กรุณากรอกใหม่อีกครั้ง"),
  confirmPassword: Yup.string()
    .oneOf([Yup.ref('password'), null], 'รหัสผ่านไม่ตรงกัน กรุณากรอกใหม่อีกครั้ง')
    .transform((x) => (x === "" ? undefined : x))
    // password optional in edit mode
    .concat(user ? null : Yup.string().required("กรุณากรอกยืนยันรหัสผ่าน"))
    .min(8, "รหัสผ่านจะต้องมีอย่างน้อย 8 ตัวอักษร, กรุณากรอกใหม่อีกครั้ง"),
});
...
async function onSubmit(values) {
  try {
    ...
    await usersStore.create(JSON.stringify(values));

    if(usersStore.get_res_code === 'WA00000'){
      message = "สร้างบัญชีผู้ดูแลระบบเสร็จสิ้น";
      await router.push("/admin-management");
      alertStore.success(message);
    } else {
      message = '"' + usersStore.get_res_desc + '"' + " การสร้างบัญชีผู้ดูแลระบบเกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง";
      await router.push("/admin-management");
...
<template>
  ...
  <Breadcrumb link_menu="/admin-management" name_menu="จัดการผู้ดูแลระบบ" title="สร้าง" />
          ...
          <Field name="profilePicture" v-model="profilePicture" type="hidden" />
            ...
            <div class="form-row mt-56 lg:mt-0 grid grid-cols-1 lg:grid-cols-1 gap-5">
              <div class="form-group grid grid-cols-2">
                <label class="mb-3" :class="{ 'is-invalid': errors.username }">ชื่อผู้ใช้ (ภาษาอังกฤษ) <span class="required">*</span></label>
                <div class="flex flex-col">
                  ...
                  />
                  <div class="invalid-feedback mt-2">{{ errors.username }}</div>
                </div>
              </div>
              <div class="form-group grid grid-cols-2">
              ...
              <div class="form-group grid grid-cols-2">
                 <label class="mb-2" :class="{ 'is-invalid': errors.password }">
                รหัสผ่าน <span class="required">*</span></label>
                  <div class="flex flex-col">
                     <Field
                      name="password"
                      type="password"
                      ...
                      />
                      <div class="invalid-feedback mt-2">{{ errors.password }}</div>
                </div>
              </div>
              <div class="form-group grid grid-cols-2">
