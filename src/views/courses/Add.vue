...
async function onSubmit(values) {
  try {
    ...
    await coursesStore.create(JSON.stringify(values));
    message = "สร้างคอร์สอบรมเสร็จสิ้น";
...
<template>
    ...
    <Breadcrumb link_menu="/courses" name_menu="คอร์สอบรม" title="เพิ่มคอร์สอบรม" />
      ...
      <div class="card-body">
        ...
        <div class="form-row grid grid-cols-1 gap-5 mb-3">
          <div class="form-group flex flex-col">
            ...
            <Field as="select" v-model="courseType" id="courseType" name="courseType" class="select select-sm select-bordered bg-transparent" :class="{ 'is-invalid': errors.courseType }">
        ...
        <div class="form-row grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
            <div class="form-group flex flex-col" v-if="courseType === 'ONLINE' || courseType === 'BOTH'">
          ...
          <div class="flex gap-3">
            <div class="form-group flex flex-col flex-1" v-if="courseType === 'ONSITE' || courseType === 'BOTH'">
              <label class="mb-3" :class="{ 'is-invalid': errors.courseOnsiteDate }">วันอบรม ออนไซต์ <span class="required">*</span></label>
              <Field class="input-sm" v-model="courseOnsiteDate" type="text" name="courseOnsiteDate" v-slot="{ fieldOnsite }">
